...
public class ProductsListViewModel : ViewModel
{
    ...
    ISearchProductsCountDB searchProductsCountDB;
    ISearchProductQueryDB searchProductQueryDB;

    public ProductsListViewModel(MainViewModel mainViewModel, 
                                 IManufacturerListDB manufacturerListDB, 
                                 IProductsRemoveDB productsRemoveDB, 
                                 ISaveProductDB saveProductDB,
                                 ISearchProductsCountDB searchProductsCountDB,
                                 ISearchProductQueryDB searchProductQueryDB)
    {
        ...
        try
        {
            Manufacturers = manufacturerListDB.ManufacturerList();
        }
        catch
        {
        ...
        SearchForProducts(searchProductsCountDB, searchProductQueryDB);
        ...
        RemoveProductCommand = new(() =>
        {
            ...
            try
            {
                productsRemoveDB.ProductRemove(SelectedProduct);
                saveProductDB.SaveProduct(); 
    ...
    private void SearchForProducts(ISearchProductsCountDB searchProductsCountDB, ISearchProductQueryDB searchProductQueryDB)
    {
        try
        {
            var countAll = searchProductsCountDB.SearchProducts();
            var productsQuery = searchProductQueryDB.SearchProductQuery(SearchText); 

            if (SelectedManufacturer.Id != 0)
                productsQuery = (List<Product>)productsQuery.Where(s => s.ProductManufacturerId == SelectedManufacturer.Id);

            productsQuery = SelectedCostSortType switch
            {
                "По возрастанию" => (List<Product>)productsQuery.OrderBy(s => s.ProductCost),
                "По Убыванию" => (List<Product>)productsQuery.OrderByDescending(s => s.ProductCost),
                _ => productsQuery,
